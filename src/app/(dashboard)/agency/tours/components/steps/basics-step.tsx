...
function photoToItem(p: { id: string; previewUrl: string }): PhotoItem {
  return { id: p.id, previewUrl: p.previewUrl };
}
...
export function BasicsStep({ data, onUpdate, errors }: BasicsStepProps) {
  ...
  const photos: PhotoItem[] = (data.photos ?? []).map(photoToItem);
  ...
  const addFiles = useCallback(
    (files: FileList | null) => {
      ...
      if (newPhotos.length > 0) {
        onUpdate({
          photos: [...photos, ...newPhotos].map((p) => ({ id: p.id, previewUrl: p.previewUrl })),
        });
      }
    },
    [photos, onUpdate]
  ...
  const removePhoto = (id: string) => {
    ...
    onUpdate({
      photos: photos.filter((p) => p.id !== id).map((p) => ({ id: p.id, previewUrl: p.previewUrl })),
    });
  };
  ...
  const movePhoto = (fromIndex: number, toIndex: number) => {
    ...
    onUpdate({ photos: arr.map((p) => ({ id: p.id, previewUrl: p.previewUrl })) });
  };
  ...
  const handlePreviewDrop = (e: React.DragEvent, toIndex: number) => {
    e.preventDefault();
    setDragOverIndex(null);
    setDraggingIndex(null);
    const fromIndex = parseInt(e.dataTransfer.getData(REORDER_DATA_TYPE) ?? e.dataTransfer.getData("text/plain") ?? "", 10);
    ...
    >
      ...
      <div className="mb-6">
        ...
        <Textarea
          ...
          onChange={(e) => onUpdate({ shortDescription: e.target.value || undefined })}
        />
      ...
      {/* Photos - full-width drop zone, previews below */}
          ...
          <div className="mt-4 flex flex-wrap gap-3">
            ...
            <div
              ...
              className={cn(
                ...
                dragOverIndex === index && draggingIndex !== index && "ring-2 ring-primary ring-offset-2"
              )}
            >
              <img
              ...
              <div className="absolute bottom-2 left-2 flex gap-2" onPointerDown={(e) => e.stopPropagation()}>
                {index > 0 && (
                  <button
                    type="button"
