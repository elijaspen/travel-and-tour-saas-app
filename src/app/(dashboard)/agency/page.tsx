...
import { BarChart2, Calendar, Package, Plus, Star, TrendingDown, TrendingUp } from "lucide-react";
...
export default async function BusinessDashboardPage() {
  await requireRole([ProfileRoles.BUSINESS_OWNER, ProfileRoles.AGENT, ProfileRoles.ADMIN]);
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        />
        <StatCard title="Avg. Rating" value={avgRating} icon={<Star className="h-4 w-4" />} />
      </div>
        ...
        <Card className="h-full">
          <CardHeader className="flex flex-row items-start justify-between pb-4">
            <div>
              ...
              <p className="mt-0.5 text-sm text-muted-foreground">Monthly performance comparison</p>
            </div>
            ...
            {ACTIVE_PROMOTIONS.map((promo) => (
              <div key={promo.id} className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Switch defaultChecked={promo.active} aria-label={`Toggle ${promo.code}`} />
                </div>
              </div>
            ))}
      ...
      <div className="grid gap-4 lg:grid-cols-[1fr_320px]">
        <Card className="h-full">
          <CardHeader className="flex flex-row items-center justify-between pb-4">
            <div>
              ...
              <p className="mt-0.5 text-sm text-muted-foreground">Latest customer reservations</p>
            </div>
              ...
              <TableBody>
                {RECENT_BOOKINGS.map((booking) => (
                  <TableRow key={booking.id}>
                    ...
                    <TableCell className="text-sm text-muted-foreground">{booking.dates}</TableCell>
                    <TableCell>
                      <Badge
                        variant="secondary"
        ...
        <Card className="h-full">
          <CardHeader className="pb-4">
            ...
            <p className="text-sm text-muted-foreground">Best selling tours this month</p>
          </CardHeader>
          <CardContent className="flex flex-col gap-4">
            ...
            <Button asChild variant="outline" size="sm" className="mt-1 w-full text-xs">
              <Link href={ROUTE_PATHS.AUTHED.AGENCY.TOURS}>View All Packages</Link>
            </Button>
          </CardContent>
        </Card>
      </div>
